...
import { Block, BlockDef } from "./writer/Block";

type BlockKey = "a" | "b" | "c" | "d" | "e" | "f";

type BlockDefinition = {
  readonly consonantKeys: Set<BlockKey>;
  readonly vowelKeys: Set<BlockKey>;
};

export default function App() {
  const reduce = (
    prevDefinition: BlockDefinition,
    action: { type: "toggle"; blockKey: BlockKey; shift: boolean }
  ) => {
    const newBlockDefinition: BlockDefinition = {
      consonantKeys: new Set<BlockKey>(prevDefinition.consonantKeys),
      vowelKeys: new Set<BlockKey>(prevDefinition.vowelKeys)
    };

    switch (action.type) {
      case "toggle": {
        const keys = action.shift
          ? newBlockDefinition.vowelKeys
          : newBlockDefinition.consonantKeys;

        keys.has(action.blockKey)
          ? keys.delete(action.blockKey)
          : keys.add(action.blockKey);
        break;
      }
      default:
        throw new Error();
  ...
  const [blockDefnition, dispatch] = useReducer(reduce, {
    consonantKeys: new Set<BlockKey>(),
    vowelKeys: new Set<BlockKey>()
  });
  ...
  const handleKeyPress = (event: KeyboardEvent) => {
    ...
    if (blockKey) {
      dispatch({ type: "toggle", blockKey, shift: event.shiftKey });
    }
  };
  ...
  const blockDef: BlockDef = [
    Array.from(blockDefnition.vowelKeys).sort().join(""),
    Array.from(blockDefnition.consonantKeys).sort().join("")
  ].join("|") as BlockDef;

  return <Block definition={blockDef} label="test label" />;
}
...
// const ConsonantSound = ({ sound }: { sound: string }) => (
//   <Block edges={toEdgeSet(consonantSounds[sound])} label={sound} />
// );

// const VowelSound = ({ sound }: { sound: string }) => (
//   <Block edges={toEdgeSet(vowelSounds[sound])} label={sound} />
// );
...
//   <Block
//     edges={toEdgeSet(
//       `${consonantSounds[consonantSound]}-${vowelSounds[vowelSound]}`
//     )}
//     label={`${consonantSound}${vowelSound}`}
//   />
// );
...
//           <span color="red">
//             <SoundWord sounds="D,ay-N-J,err" text="danger" />
//           </span>
//         </div>
//       </>
//     ],
...
//             !<br />
//             <Word
//               blockDefinitions={["AD-BD-BE-DF-HK-IL-JL-LM", "AB-BE-HK-KM-LM"]}
//               text="???"
//             />
//             <Word
//               blockDefinitions={["AD-BD-BE-DF-HK-IL-JL-LM", "AB-BE-HK-KM-LM"]}
...
//             <SoundWord sounds="B,u-T,i-N" text="button" />
//             <SoundWord sounds="T,oo" text="to" />
//             <SoundWord sounds="R,u-N" text="run" />. TODO
//           </div>
//           <br />
//           <div className="section" title="(A) speech bubble">
//             <SoundWord sounds="P-R,e-S" text="press" />
...
//           <SoundWord sounds="W,i-Dh" text="with" />{" "}
//           {/* Why is `Dh` not rendering when it's on it's own? */}
//           6
...
//   const selectedSounds = [selectedConsonant, selectedVowel]
//     .filter(Boolean)
//     .join(",");

//   const selectedConsonantEdges =
//     selectedConsonant && consonantSounds[selectedConsonant];
//   const selectedVowelEdges = selectedVowel && vowelSounds[selectedVowel];

//   const selectedEdges: string[] = [];
//   if (selectedConsonantEdges) {
//     selectedEdges.push(selectedConsonantEdges);
//   }
//   if (selectedVowelEdges) {
//     selectedEdges.push(selectedVowelEdges);
//   }
//   const selectedEdgeSet = selectedEdges.length
//     ? toEdgeSet(selectedEdges.join("-"))
//     : new Set<Edge>();

//   const [previewEdgeSets, setPreviewEdgeSets] = React.useState<
//     Array<Set<Edge>>
//   >([]);
...
//             ))}
//             <li>
//               <button onClick={() => setSelectedConsonant(undefined)}>X</button>
//             </li>
//           </ol>
//           <hr />
//           Vowels
...
//             ))}
//             <li>
//               <button onClick={() => setSelectedVowel(undefined)}>X</button>
//             </li>
//           </ol>
//           <hr />
//           <div className="preview">
//             {previewEdgeSets.map((edgeSet, index) => (
//               <Block
//                 key={index}
//                 edges={edgeSet}
//                 label={previewSounds[index] || "_"}
//               />
//             ))}
//             {
//               <Block
//                 edges={selectedEdgeSet ?? new Set()}
//                 label={selectedSounds || "_"}
//               />
//             }
//             <button onClick={handleNextClick}>➡️</button>
//           </div>
//           <pre>{`<SoundWord sounds="${previewSounds.join(
//             "-"
//           )}" text="" />\n`}</pre>
...
//           const padPageNumber = (pageNumber: number): string =>
//             pageNumber.toString().padStart(2, "0");

//           return (
//             <li key={pagePairIndex}>
//               <div className="page-pair">
//                 <img
//                   id={padPageNumber(leftPageNumber)}
//                   className="left-page"
//                   src={`images/manual/page${padPageNumber(leftPageNumber)}.jpg`}
//                 />
//                 <img
//                   id={padPageNumber(rightPageNumber)}
//                   className="right-page"
//                   src={`images/manual/page${padPageNumber(
//                     rightPageNumber
//                   )}.jpg`}
//                 />
//               </div>
